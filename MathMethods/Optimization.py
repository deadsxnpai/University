...
def math_model(F, TT):
    ...
    def k1(TT): return A1 * np.exp((-E1) / (R * TT))
    def k2(TT): return A2 * np.exp((-E2) / (R * TT))
    def k3(TT): return A3 * np.exp((-E3) / (R * TT))
    def k4(TT): return A4 * np.exp((-E4) / (R * TT))

    def d_C3(C1, C2, C3, C6, t): return k1(t)*C1*C2-k4(t)*C3*C6
    def d_C1(C1, C2, t): return -k1(t)*C1*C2-k2(t)*C1*C2
    def d_C2(C1, C2, C3, C6, t): return -k1(t)*C1*C2-k2(t)*C1*C2+k4(t)*C3*C6
    def d_C6(C4, C5, C3, C6, t): return k3(t)*C4*C5-k4(t)*C3*C6
    def d_C4(C1, C2, C4, C5, t): return k2(t)*C1*C2-k3(t)*C4*C5
    def d_C5(C3, C6, C4, C5, t): return -k3(t)*C4*C5 + k4(t)*C3*C6
    def d_T(d_C1, d_C2, d_C3, d_C6, T, F): return (k1(T)*d_C1*d_C2*Q1*V +k2(T)*d_C1*d_C2*Q2*V+k4(T)*d_C3*d_C6*Q4*V-K_t*F*(T-TT)) / (ro*V*C_t)
...
def minimum():
    ...
    for i in F_range:
        ...
        C_list.append(tempC)
    min_time = min(
        el for el in Time_list if Temp_list[Time_list.index(el)]-273 < 60)
...
def lab_6():
    ...
    for k in range(1, N - 9):
        ...
        for i in np.arange(k, k + Ns):
            j += xi[i] * np.sqrt(disp0 / (dispx * -alpha0)) * \
                np.exp(alpha0 * (i - k)) + M0
        z.append(1 / Ns * j)
...
def plot(Time, y, C):
    fig, axes = plt.subplots(
        nrows=1, ncols=2, figsize=(15, 6), label='Coursework')
...
def plot3D(F, t, C):
    ...
    axes.plot_trisurf(F, t, C, cmap='plasma',
